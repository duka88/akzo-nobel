...
gulp.task('iconfont-build', () => {
	return gulp.src([`${svgPath}/*.svg`])
		.pipe(iconfontCss({
			...
			cacheBuster: globalVars.mode === 'production' ? '' : `?v=${new Date().getTime()}`,
...
gulp.task('fonts-to-web', (done) => {
	if(!globalVars.webFolder) done();

	return gulp.src(['dist/assets/fonts/*'])
		.pipe(gulpif(!!globalVars.webFolder, gulp.dest(`${globalVars.webFolder}/assets/fonts`)));
});

gulp.task('iconfont', gulp.series('optimize-svgs', 'iconfont-build', 'fonts-to-web'));
